...
The deployment script supports dynamic namespace and environment:

```bash
cd agentgateway-helm

# Deploy to default namespace (agentgateway-system) and dev environment
./helm-deployment.sh

# Deploy to custom namespace and environment
./helm-deployment.sh my-namespace prod

# Supported environments: dev (default), prod
```
...
### Upgrade
To apply changes after modifying `values.yaml` or other chart files:

```bash
cd agentgateway-helm

# Update dependencies if Chart.yaml was modified
helm dependency update

# Upgrade using the script (preserves namespace and environment)
./helm-deployment.sh [namespace] [environment]

# Or manual upgrade
helm upgrade agentgateway-dev . --namespace agentgateway-system --wait

# Or for production
helm upgrade agentgateway-prod . -f values-prod.yaml --namespace agentgateway-prod --wait
```

Re-running the deployment script will upgrade the existing release.
