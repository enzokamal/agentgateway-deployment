...
- ✅ Deploys the ADK UI application
...
The ADK UI is deployed automatically in Step 1. Access it at `http://localhost:5000` after the script completes.
...
kubectl port-forward svc/adk-ui-service 5000:5000 -n agentgateway-system
...
- **ADK UI Application**: `http://localhost:5000`
...
```bash
# Check UI deployment
kubectl get deployment adk-ui -n agentgateway-system

# Check UI pod logs
kubectl logs -l app=adk-ui -n agentgateway-system

# Check UI service
kubectl get service adk-ui-service -n agentgateway-system

# Check HTTP route
kubectl get httproute adk-ui -n agentgateway-system
```
...
The ADK UI is deployed automatically by the main script. To update:
...
# Or update manually
kubectl set image deployment/adk-ui adk-ui=new-image:tag -n agentgateway-system
kubectl rollout restart deployment/adk-ui -n agentgateway-system
```
...
```bash
# Scale ADK UI
kubectl scale deployment adk-ui --replicas=3 -n agentgateway-system
...
kubectl logs -f -l app=adk-ui -n agentgateway-system
```
...
```bash
# Remove ADK UI
kubectl delete httproute adk-ui -n agentgateway-system
kubectl delete service adk-ui-service -n agentgateway-system
kubectl delete deployment adk-ui -n agentgateway-system
kubectl delete serviceaccount adk-ui-sa -n agentgateway-system
...
- ✅ **ADK Web UI**: Modern chat interface for MCP server interaction
